...
namespace AllIn1SpringsToolkit.Demo.Scripts
{
    ...
    public class HpBarSpringDemo : DemoElement
    {
        ...
        public override void Initialize(bool hideUi)
        {
            ...
            hpGreenColorSpringComponent.SetTargetAndReachEquilibrium(hpColorGradient.Evaluate(currentHp));
            backBarColorSpringComponent.SetTargetAndReachEquilibrium(backBarImage.color);
            
            leftHpBarSpringComponent.SetTargetAndReachEquilibrium(leftHpBarRectTransform.anchoredPosition);
            rightHpBarSpringComponent.SetTargetAndReachEquilibrium(rightHpBarRectTransform.anchoredPosition);
        }
        ...
        public void AddRemoveHp(float amount)
        {
            ...
            if(sign < 0)
            {
                hpGreenSliderSpringComponent.SetTargetImmediately(currentHp);
                hpWhiteSliderSpringComponent.SetTarget(currentHp);
            }
            else
            {
                hpWhiteSliderSpringComponent.SetTargetImmediately(currentHp);
            ...
            if(currentHp < 0.01f)
            {
                scaleSpringComponent.AddVelocityToScaleSpring(new Vector3(sign * scaleVelOnHealthLimits, 0f, 0f));
            ...
            else scaleSpringComponent.AddVelocityToScaleSpring(new Vector3(1f, 2f, 0f) * (amount * scaleVelOnHealthChange));
        }
