...
namespace AllIn1SpringsToolkit
{
	...
	public class TransformSpringComponentCustomEditor : SpringComponentCustomEditor 
	{
		...
		//Enable Springs
		private SerializedProperty spSpringPositionEnabled;
		private SerializedProperty spSpringScaleEnabled;
		private SerializedProperty spSpringRotationEnabled;
		...
		protected override void RefreshSerializedProperties()
		{
			...
			spSpringPositionEnabled = serializedObject.FindProperty("springPositionEnabled");

			spSpringScaleEnabled = serializedObject.FindProperty("springScaleEnabled");

			spSpringRotationEnabled = serializedObject.FindProperty("springRotationEnabled");
		...
		{
			DrawSpring("Position Spring", spPositionSpring, spSpringPositionEnabled, springPositionDrawer);
			DrawSpring("Rotation Spring", spRotationSpring, spSpringRotationEnabled, springRotationDrawer);
			DrawSpring("Scale Spring", spScaleSpring, spSpringScaleEnabled, springScaleDrawer);
		}
		protected override void DrawMainAreaUnfolded()
		{
			DrawSerializedProperty(spFollowerTransform, LABEL_WIDTH);

			DrawSerializedProperty(spUseTransformAsTarget, LABEL_WIDTH);

			if (spUseTransformAsTarget.boolValue)
			{
				DrawSerializedProperty(spTargetTransform, LABEL_WIDTH);
			}

			DrawSerializedProperty(spSpaceType, LABEL_WIDTH);
		}

		private void DrawSpring(string springName, SerializedProperty spSpring, SerializedProperty spSpringEnabled, SpringDrawer springDrawer)
		{
			springDrawer.springEditorObject.Unfolded = DrawRectangleArea(areaName: springName, spSpringEnabled, springDrawer.springEditorObject.Unfolded);
		...
		protected override void DrawCustomInitialValuesSection()
		{
			DrawInitialValuesBySpring(
				...
				width: LABEL_WIDTH,
				springEnabled: spSpringPositionEnabled.boolValue, 
			...
			DrawInitialValuesBySpring(
				...
				width: LABEL_WIDTH,
				springEnabled: spSpringRotationEnabled.boolValue,
			...
			DrawInitialValuesBySpring(
				...
				width: LABEL_WIDTH,
				springEnabled: spSpringScaleEnabled.boolValue,
