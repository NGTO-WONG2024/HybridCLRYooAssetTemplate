...
namespace AllIn1SpringsToolkit
{
    ...
    public class ShaderFloatSpringComponentCustomEditor : SpringComponentCustomEditor
    {
        ...
        {
            DrawSerializedProperty(spShaderPropertyName, LABEL_WIDTH);
            DrawSerializedProperty(spTargetIsRenderer, LABEL_WIDTH);

            if (spTargetIsRenderer.boolValue)
            {
                DrawSerializedProperty(spTargetRenderer, LABEL_WIDTH);
            }
            else
            {
                DrawSerializedProperty(spTargetGraphic, LABEL_WIDTH);
            }

            DrawSerializedProperty(spGetAutoUpdatedMaterialFromTarget, LABEL_WIDTH);
            
            if (!spGetAutoUpdatedMaterialFromTarget.boolValue)
            {
                DrawSerializedProperty(spAutoUpdatedMaterial, LABEL_WIDTH);
            }
        }

        protected override void DrawCustomInitialValuesSection()
        {
            DrawInitialValuesBySpring("Initial Values", LABEL_WIDTH, shaderValueSpringDrawer);
        }

        protected override void DrawCustomInitialTarget()
        {
            DrawCustomTargetBySpring("Target", LABEL_WIDTH, shaderValueSpringDrawer);
        }
