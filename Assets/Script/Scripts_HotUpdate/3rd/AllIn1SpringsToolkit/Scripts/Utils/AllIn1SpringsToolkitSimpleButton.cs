        ...
        private void HandleSelectionState(SelectionState state)
        {
            switch(state)
            {
                ...
                case SelectionState.Normal:
					if (doScaleSpring)
					{
						transformSpringComponent.SetScaleTarget(originalScale);
					}
					if (doRotationSpring)
					{
						transformSpringComponent.SetRotationTarget(originalRotation);
					}
					break;
                ...
                case SelectionState.Selected:
                    if (doScaleSpring)
					{
						transformSpringComponent.SetScaleTarget(highlightedScale);
					}
					if (doRotationSpring)
					{
						transformSpringComponent.SetRotationTarget(Quaternion.Euler(highlightedRotation));
					}
                    break;
                ...
                case SelectionState.Pressed:
					if (doScaleSpring)
					{
						transformSpringComponent.AddVelocityToScaleSpring(pressedScaleVelocityAdd);
					}
					if (doRotationSpring)
					{
                        transformSpringComponent.AddVelocityToRotationSpring(pressedRotationVelocityAdd);
					}
                    break;
