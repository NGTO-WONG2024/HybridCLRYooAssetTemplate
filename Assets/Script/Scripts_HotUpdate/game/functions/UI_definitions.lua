...
function G.UIDEF.card_focus_ui(card)
    ...
    if G.hand and card.area == G.hand then
        ease_value(playing_card_colour, 4, -1.5, nil, 'REAL', nil, 0.2, 'quad')
    end
    ...
    local base_background = UIBox {
        ...
        definition = (not G.hand or card.area ~= G.hand) and { n = G.UIT.ROOT, config = { align = 'cm', minw = card_width + 0.3, minh = card.T.h + 0.3, r = 0.1, colour = adjust_alpha(G.C.BLACK, 0.7), outline_colour = lighten(G.C.JOKER_GREY, 0.5), outline = 1.5, line_emboss = 0.8 }, nodes = {
    ...
    if (card.area == G.jokers and G.jokers or card.area == G.consumeables and G.consumeables) and G.STATE ~= G.STATES.TUTORIAL then
        --Add a sell button
...
function G.UIDEF.deck_preview(args)
    ...
    for k, v in ipairs(G.playing_cards) do
        ...
        if (v.area and v.area == G.deck) or v.ability.wheel_flipped then
            if v.ability.wheel_flipped then
            ...
            if v.ability.effect == 'Stone Card' then
                ...
                for kk, vv in pairs(suit_counts) do
                    if v.base.suit == kk then
                        suit_counts[kk] = suit_counts[kk] + 1
                    end
                    if v:is_suit(kk) then
    ...
    for k, v in ipairs(suit_map) do
        ...
        if mod_suit_counts[v] ~= suit_counts[v] then
            mod_suit_diff = true
        end

        suit_labels[#suit_labels + 1] = { n = G.UIT.R, config = { align = "cm", r = 0.1, padding = 0.03, colour = G.C.JOKER_GREY }, nodes = {
...
function create_shop_card_ui(card, type, area)
    G.E_MANAGER:add_event(Event({
        ...
        func = (function()
            ...
            if card.ability.consumeable then
                --and card:can_use_consumeable(true, true)
...
function name_from_rows(name_nodes, background_colour)
    if not name_nodes or (type(name_nodes) ~= 'table') or not next(name_nodes) then
        return
    end
...
function G.UIDEF.card_h_popup(card)
    if card.ability_UIBox_table then
        ...
        local card_type_background = (AUT.card_type == 'Locked' and G.C.BLACK) or
        ...
        local disp_type, is_playing_card = (AUT.card_type ~= 'Locked' and AUT.card_type ~= 'Undiscovered' and AUT.card_type ~= 'Default') or debuffed,
        ...
        if AUT.info then
            for k, v in ipairs(AUT.info) do
                info_boxes[#info_boxes + 1] = { n = G.UIT.R, config = { align = "cm" }, nodes = {
...
function create_popup_UIBox_tooltip(tooltip)
    ...
    local t = {
        n = G.UIT.ROOT, config = { align = "cm", padding = 0.05, r = 0.1, colour = G.C.RED, emboss = 0.05 }, nodes = { { n = G.UIT.C, config = { align = "cm", padding = 0.05, r = 0.1, colour = G.C.WHITE, emboss = 0.05 }, nodes = rows } } }
    return t
end
...
function create_UIBox_blind_select()
    ...
    G.GAME.blind_on_deck = not (G.GAME.round_resets.blind_states.Small == 'Defeated' or G.GAME.round_resets.blind_states.Small == 'Skipped' or G.GAME.round_resets.blind_states.Small == 'Hide') and 'Small' or
...
function create_UIBox_blind_tag(blind_choice, run_info)
    ...
    if not G.GAME.round_resets.blind_tags[blind_choice] then
        return nil
    end
...
function create_UIBox_blind_choice(type, run_info)
    ...
    local t = { n = G.UIT.R, config = { id = type, align = "tm", func = 'blind_choice_handler', minh = not run_info and 10 or nil, ref_table = { deck = nil, run_info = run_info }, r = 0.1, padding = 0.05 }, nodes = {
...
function create_toggle(args)
    ...
    local t = { n = args.col and G.UIT.C or G.UIT.R, config = { align = "cm", padding = 0.1, r = 0.1, colour = G.C.CLEAR, focus_args = { funnel_from = true } }, nodes = {
...
function create_option_cycle(args)
    ...
    local t = { n = G.UIT.C, config = { align = "cm", padding = 0.1, r = 0.1, colour = G.C.CLEAR, id = args.id and (not args.label and args.id or nil) or nil, focus_args = args.focus_args }, nodes = {
    ...
    if args.cycle_shoulders then
        t = { n = G.UIT.R, config = { align = "cm", colour = G.C.CLEAR }, nodes = {
        ...
        t = { n = G.UIT.R, config = { align = "cm", colour = G.C.CLEAR, padding = 0.0 }, nodes = {
            t
        } }
    end
...
function create_tabs(args)
    ...
    local t = { n = G.UIT.R, config = { padding = 0.0, align = "cm", colour = G.C.CLEAR }, nodes = {
...
function create_text_input(args)
    ...
    local t = { n = G.UIT.C, config = { align = "cm", draw_layer = 1, colour = G.C.CLEAR }, nodes = {
        { n = G.UIT.C, config = { id = 'text_input', align = "cm", padding = 0.05, r = 0.1, draw_layer = 2, hover = true, colour = args.colour, minw = args.w, min_h = args.h, button = 'select_text_input', shadow = true }, nodes = {
            { n = G.UIT.R, config = { ref_table = args, padding = 0.05, align = "cm", r = 0.1, colour = G.C.CLEAR }, nodes = {
                { n = G.UIT.R, config = { ref_table = args, align = "cm", r = 0.1, colour = G.C.CLEAR, func = 'text_input' }, nodes = ui_letters
                }
            } }
        } }
    } }
...
function create_dynatext_pips(args)
    ...
    if not (args.dynatext) or not (#args.dynatext.strings > 1) then
        return
    end
    ...
    local t = { n = G.UIT.ROOT, config = { align = "cm", minw = G.ROOM.T.w * 5, minh = G.ROOM.T.h * 5, padding = 0.15, r = 0.1, colour = { G.C.BLACK[1], G.C.BLACK[2], G.C.BLACK[3], 0.6 } }, nodes = {
        { n = G.UIT.C, config = { align = "cm", minh = 1, r = 0.3, padding = 0.1, minw = 1, colour = G.C.WHITE, shadow = true }, nodes = {
            { n = G.UIT.C, config = { align = "cm", minh = 1, r = 0.2, padding = 0.2, minw = 1, colour = G.C.CLEAR, outline = 1, outline_colour = G.C.BLACK }, nodes = {
                { n = G.UIT.R, config = { align = "cm", padding = 0.05 }, nodes = nodes
                },
...
function create_UIBox_usage(args)
    ...
    for i = 1, 10 do
        ...
        if v then
            ...
            histograms[#histograms + 1] = { n = G.UIT.C, config = { align = "bm", minh = 6.2, colour = G.C.UI.TRANSPARENT_DARK, r = 0.1 }, nodes = {
    ...
    local t = { n = G.UIT.ROOT, config = { align = "cm", minw = 3, padding = 0.1, r = 0.1, colour = G.C.UI.TRANSPARENT_DARK }, nodes = {
        { n = G.UIT.R, config = { align = "cm", padding = 0.1 }, nodes = {
            ...
            { n = G.UIT.T, config = { text = _type == 'joker_usage' and localize('ph_stat_joker') or
...
function create_progress_box(_profile_progress, smaller)
    ...
    _profile_progress.overall_tally, _profile_progress.overall_of = _profile_progress.discovered.tally / _profile_progress.discovered.of +
...
function create_UIBox_high_scores_row(score)
    if not G.PROFILES[G.SETTINGS.profile].high_scores[score] then
        return nil
    end
...
function create_UIBox_hand_tip(handname)
    if not G.GAME.hands[handname].example then
        return { n = G.UIT.R, config = { align = "cm" }, nodes = {} }
    end
    local cardarea = CardArea(
            2, 2,
...
function create_UIBox_current_hands(simple)
    ...
    local t = { n = G.UIT.ROOT, config = { align = "cm", minw = 3, padding = 0.1, r = 0.1, colour = G.C.CLEAR }, nodes = {
        { n = G.UIT.R, config = { align = "cm", padding = 0.04 }, nodes = hands
        },
    } }
...
function G.UIDEF.deck_stake_column(_deck_key)
    ...
    for i = #G.P_CENTER_POOLS['Stake'], 1, -1 do
        ...
        if i > 1 then
            stake_col[#stake_col + 1] = { n = G.UIT.R, config = { align = "cm", minh = 0.1, minw = 0.04 }, nodes = {} }
        end
    end
...
function create_UIBox_your_collection_editions()
    ...
    G.your_collection[1] = CardArea(
            ...
            1.03 * G.CARD_H,
            { card_limit = 5, type = 'title', highlight_limit = 0, collection = true })
    local deck_tables = { n = G.UIT.R, config = { align = "cm", padding = 0, no_fill = true }, nodes = {
    ...
    for i = 1, 5 do
        ...
        if G.P_CENTERS['e_' .. editions[i]].discovered then
...
function create_UIBox_your_collection_decks()
    ...
    local t = create_UIBox_generic_options({ back_func = 'your_collection', contents = {
        create_option_cycle({ options = ordered_names, opt_callback = 'change_viewed_back', current_option = 1, colour = G.C.RED, w = 4.5, focus_args = { snap_to = true }, mid = { n = G.UIT.R, config = { align = "cm", minw = 2.5, padding = 0.1, r = 0.1, colour = G.C.BLACK, emboss = 0.05 }, nodes = {
...
function create_UIBox_blind_popup(blind, discovered, vars)
    ...
    if discovered then
        ...
        blind_text[#blind_text + 1] = { n = G.UIT.R, config = { align = "cm", emboss = 0.05, r = 0.1, minw = 2.5, padding = 0.07, colour = G.C.WHITE }, nodes = {
        ...
        blind_text[#blind_text + 1] = { n = G.UIT.R, config = { align = "cm", emboss = 0.05, r = 0.1, minw = 2.5, padding = 0.1, colour = G.C.WHITE }, nodes = {
            { n = G.UIT.R, config = { align = "cm" }, nodes = {
...
function create_UIBox_deck_unlock(deck_center)
    ...
    for k, v in ipairs(deck_criteria) do
        if k > 1 then
            deck_criteria_cols[#deck_criteria_cols + 1] = { n = G.UIT.C, config = { align = "cm", padding = 0, minw = 0.1 }, nodes = {} }
        end
...
function G.UIDEF.challenge_list_page(_page)
    ...
    for k, v in ipairs(G.CHALLENGES) do
        if k > G.CHALLENGE_PAGE_SIZE * (_page or 0) and k <= G.CHALLENGE_PAGE_SIZE * ((_page or 0) + 1) then
            ...
            challenge_list[#challenge_list + 1] = { n = G.UIT.R, config = { align = "cm" }, nodes = {
...
function G.UIDEF.challenge_description_tab(args)
    ...
    if args._tab == 'Rules' then
        ...
        if (not start_rules[1]) and (not modded_starts) then
            start_rules[#start_rules + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = localize { type = 'text', key = 'ch_m_none', vars = {} } }
        end
        if not game_rules[1] then
            game_rules[#game_rules + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = localize { type = 'text', key = 'ch_c_none', vars = {} } }
        end
    ...
    elseif args._tab == 'Restrictions' then
        ...
        if challenge.restrictions then
            if challenge.restrictions.banned_cards then
                ...
                for k, v in ipairs(challenge.restrictions.banned_cards) do
                    ...
                    if #row_cards[_row] > max_width then
                        max_width = #row_cards[_row]
                    end
                end
        ...
        if not banned_cards[1] then
            banned_cards[#banned_cards + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = localize { type = 'text', key = 'ch_m_none', vars = {} } }
        end
        if not banned_tags[1] then
            banned_tags[#banned_tags + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = localize { type = 'text', key = 'ch_c_none', vars = {} } }
        end
        if not banned_other[1] then
            banned_other[#banned_other + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = localize { type = 'text', key = 'ch_c_none', vars = {} } }
        end
        ...
        local banned_cards = { n = G.UIT.C, config = { align = "cm", r = 0.1, colour = G.C.RED }, nodes = {
            ...
            { n = G.UIT.R, config = { align = "cm", minh = 4.1, minw = 7.33, padding = 0.05, r = 0.1, colour = G.C.WHITE }, nodes = banned_cards
            }
        } }
        ...
        local banned_tags = { n = G.UIT.C, config = { align = "cm", r = 0.1, colour = G.C.RED }, nodes = {
            ...
            { n = G.UIT.R, config = { align = "cm", minh = 4.1, minw = 1.48, padding = 0.05, r = 0.1, colour = G.C.WHITE }, nodes = banned_tags }
        } }
        ...
        local banned_other = { n = G.UIT.C, config = { align = "cm", r = 0.1, colour = G.C.RED }, nodes = {
            ...
            { n = G.UIT.R, config = { align = "cm", minh = 4.1, minw = 2, padding = 0.05, r = 0.1, colour = G.C.WHITE }, nodes = banned_other }
        } }
    ...
    if type == 'Continue' then
        ...
        if G.SAVED_GAME ~= nil then
            ...
            for k, v in pairs(G.P_CENTERS) do
                if v.name == saved_game.BACK.name then
                    viewed_deck = k
                end
            end
        ...
        type == 'Continue' and { n = G.UIT.R, config = { align = "tm", minh = 3.8, padding = 0.15 }, nodes = {
            { n = G.UIT.R, config = { align = "cm", minh = 3.3, minw = 6.8 }, nodes = {
                ...
                { n = G.UIT.R, config = { align = "cm", minh = 3.8 }, nodes = {
                    create_option_cycle({ options = ordered_names, opt_callback = 'change_viewed_back', current_option = viewed_deck, colour = G.C.RED, w = 3.5, mid = { n = G.UIT.R, config = { align = "cm", minh = 3.3, minw = 5 }, nodes = {
                        { n = G.UIT.C, config = { align = "cm", colour = G.C.BLACK, padding = 0.15, r = 0.1, emboss = 0.05 }, nodes = {
                            { n = G.UIT.C, config = { align = "cm" }, nodes = {
                                { n = G.UIT.R, config = { align = "cm", shadow = false }, nodes = {
                                    { n = G.UIT.O, config = { object = area } }
...
function UIBox_dyn_container(inner_table, horizontal, colour_override, background_override, flipped, padding)
    return { n = G.UIT.R, config = { align = "cm", padding = 0.03, colour = G.C.UI.TRANSPARENT_DARK, r = 0.1 }, nodes = {
        { n = G.UIT.R, config = { align = "cm", padding = 0.05, colour = colour_override or G.C.DYN_UI.MAIN, r = 0.1 }, nodes = {
            { n = G.UIT.R, config = { align = horizontal and "cl" or (flipped and 'bm' or "tm"), colour = background_override or G.C.DYN_UI.BOSS_DARK, minw = horizontal and 100 or 0, minh = horizontal and 0 or 30, r = 0.1, padding = padding or 0.08 }, nodes = inner_table
            } } } } }
end
...
function simple_text_container(_loc, args)
    ...
    if loc_result and type(loc_result) == 'table' then
        for k, v in ipairs(loc_result) do
            container[#container + 1] = { n = G.UIT.R, config = { align = "cm", padding = 0 }, nodes = {
